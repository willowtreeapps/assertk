...
class AssertLambdaTest {
    ...
    @Test
    fun successful_assert_lambda_returning_null_returns_success() {
        assertEquals(Result.success(null), assertThat { null }.valueOrFail)
    }

    @Test
    fun failing_assert_lambda_returns_failure() {
        val e = Exception("error")
        assertEquals(Result.failure<String>(e), assertThat { throw e }.valueOrFail)
    }

    @Test
    fun returnedValue_works_in_coroutine_test() = runTest {
        assertThat {
            asyncReturnValue()
    ...
    @Test
    fun returnedValue_exception_works_in_coroutine_test() = runTest {
        assertThat {
            asyncThrows()
